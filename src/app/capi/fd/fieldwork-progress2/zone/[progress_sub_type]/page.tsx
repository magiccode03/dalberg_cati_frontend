...
export default function ZoneProgressPage() {
  ...
  const fetchData = async () => {
    try {
      ...
      const response = await apiService.getFDFieldworkProgress(3, progressSubType);
      
      if (response.success && response.data) {
        setApiData(response.data);
      } else {
        setError('Failed to fetch zone progress data');
      }
    } catch (err: any) {
      console.error('Error fetching zone progress data:', err);
  ...
  const handleChartClick = (progressType: number, progressSubType: number) => {
    router.push(`/capi/fd/fieldwork-progress2/zone/${progressSubType}`);
  };

  const handleRowClick = (regionCode: string) => {
    router.push(`/capi/fd/fieldwork-progress2/zone/${progressSubType}/${regionCode}`);
  };
      ...
      <Container maxWidth="7xl" className="w-full max-w-9xl mx-auto">
        <div className="flex items-center justify-center min-h-[400px]">
          <div className="flex items-center space-x-2">
            ...
            <Text>Loading zone progress data...</Text>
          </div>
        </div>
      </Container>
    );
    ...
    <Container maxWidth="7xl" className="w-full max-w-9xl mx-auto">
      ...
      <div className="flex items-center gap-4 mb-8">
        ...
        <Heading level={2} className="text-2xl font-semibold text-gray-900">
          {apiData.progress_label || 'Zone Progress'}
        </Heading>
      </div>
      ...
      {apiData.data_provider && apiData.data_provider.length > 0 && (
        <ProgressTable
          ...
          columns={{
            code: { label: 'Region Code', field: 'region_code' },
            name: { label: 'Region Name', field: 'region_name' },
          }}
        />
      )}
    </Container>
