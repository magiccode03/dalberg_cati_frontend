...
export default function PCDrillDownPage() {
  const router = useRouter();
  const params = useParams();
  const progressSubType = parseInt(params.progress_sub_type as string);
  const pcCode = params.pc_code as string;
  ...
  useEffect(() => {
    if (progressSubType && pcCode) {
      fetchData();
    }
  }, [progressSubType, pcCode]);
  ...
  const fetchData = async () => {
    try {
      ...
      const response = await apiService.getFDFieldworkProgress(1, 4, pcCode);
      
      if (response.success && response.data) {
        setApiData(response.data);
      } else {
        setError('Failed to fetch PC drill-down data');
      }
    } catch (err: any) {
      console.error('Error fetching PC drill-down data:', err);
      ...
      <Container maxWidth="7xl" className="w-full max-w-9xl mx-auto">
        <div className="flex items-center justify-center min-h-[400px]">
          <div className="flex items-center space-x-2">
            ...
            <Text>Loading PC drill-down data...</Text>
          </div>
        </div>
      </Container>
    );
  ...
  return (
    <Container maxWidth="7xl" className="w-full max-w-9xl mx-auto">
      ...
      <div className="flex items-center gap-4 mb-8">
        <Button
          variant="outline"
          onClick={() => router.push(`/capi/fd/fieldwork-progress2/pc/${progressSubType}`)}
        ...
        <Heading level={2} className="text-2xl font-semibold text-gray-900">
          {apiData.sub_model?.pc_name || 'PC Drill Down'}
        </Heading>
      </div>
      ...
      {apiData.subdata_provider && apiData.subdata_provider.length > 0 && (
        <ProgressTable
          ...
          backUrl={`/capi/fd/fieldwork-progress2/pc/${progressSubType}`}
          backLabel={`Back to ${apiData.sub_model?.pc_name || 'PC Progress'}`}
