...
const UpdateMasterACPageContent = () => {
  const router = useRouter();
  const searchParams = useSearchParams();
  const acCode = searchParams.get('ac_code');

  const [formData, setFormData] = useState<MasterACData>({
    ac_code: 0,
    ac_name: '',
    total_electorate: 0,
    male: 0,
  ...
  const fetchACData = async () => {
    ...
    try {
      ...
      const response = await apiService.getMasterACForUpdate(parseInt(acCode));
      
      if (response.success && response.data) {
        const apiData = response.data;
        
        // Map API response to form data structure
        const mappedData: MasterACData = {
          ac_code: apiData.ac_code,
          ac_name: apiData.ac_name,
  ...
  const handleInputChange = (name: keyof MasterACData, value: string) => {
    setFormData(prev => ({
      ...prev,
      [name]: value
    }));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log('Updating AC data:', formData);
      
      // Simulate API call
      await new Promise(resolve => setTimeout(resolve, 1000));
      
      // Show success message and redirect back
      alert('AC data updated successfully!');
      router.push('/capi/ppm/master-data/master-ac');
      
    } catch (error) {
      console.error('Error updating AC:', error);
      alert('Failed to update AC data');
    } finally {
      setLoading(false);
    }
  };
      ...
      <Container maxWidth="7xl" className="w-full max-w-9xl mx-auto main-container">
        <div className="flex items-center justify-center min-h-screen">
          <div className="text-center">
            ...
            <p className="mt-4 text-lg">Loading AC data...</p>
          </div>
        </div>
      </Container>
    );
...
const UpdateMasterACPage = () => {
  return (
    <Suspense fallback={
      ...
      <UpdateMasterACPageContent />
    </Suspense>
  );
};

export default UpdateMasterACPage;
