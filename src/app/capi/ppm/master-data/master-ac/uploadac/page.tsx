...
export default function UploadACPage() {
  ...
  const fetchPreviousRequests = async () => {
    try {
      ...
      const response = await fetch(
        `${apiUrl}/api/capi/master-ac/upload-requests?page=${currentPage}&limit=${pageSize}`,
        {
          method: 'GET',
          headers: {
  ...
  const handleUpload = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(`${apiUrl}/api/capi/master-ac/upload`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
        },
  ...
  const handleViewRequest = (cronId: number) => {
    ...
    const apiUrl = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:4001';
    window.open(`${apiUrl}/api/capi/master-ac/view-request?cronrequest_id=${cronId}`, '_blank');
  };
  ...
  const handleDownloadUploadedFile = async (cronId: number) => {
    try {
      ...
      const response = await fetch(
        `${apiUrl}/api/capi/master-ac/download-uploaded-file?cronrequest_id=${cronId}`,
        {
          method: 'GET',
          headers: {
            'Authorization': `Bearer ${token}`,
        ...
        <form onSubmit={handleUpload}>
          ...
          <div className="mb-4">
            <Text className="text-sm text-red-600 dark:text-red-400">
              Note - Please Upload Complete AC List as per sample file
            </Text>
          </div>
