...
export default function CreateCronJobPage() {
  ...
  useEffect(() => {
    // If cloning, fetch the job to clone
    if (cloneId) {
      fetchJobToClone(cloneId);
    } else {
  ...
  const fetchJobToClone = async (id: string) => {
    try {
      const response = await apiClient.get(`/cron-jobs/${id}`);
      ...
      setError('Failed to load job for cloning');
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err.response?.data?.message || 'Failed to create cron job');
    } finally {
      setLoading(false);
    }
  };
                          ...
                          <div>
                            <p className="text-sm font-medium text-red-800 dark:text-red-200">
                              ...
                              {cronValidation.error && (
                                <span className="text-xs text-red-700 dark:text-red-300 mt-1">
                                  {cronValidation.error}
                                </span>
                              )}
                            </p>
