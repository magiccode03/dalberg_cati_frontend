...
interface APIResponse {
  success: boolean;
  summary: {
    overall_total_assigned: number;
    overall_total_complete: number;
    overall_total_pending: number;
  };
...
export default function QCUserProgressPage() {
  ...
  const fetchQCUserOptions = async (status: string) => {
    try {
      ...
      const response = await Promise.race([
        apiClient.get(`/teleform-users?data_entry=1&status=${status}&limit=500`),
        timeoutPromise
      ]) as any;
  ...
  const fetchData = async (page: number = 1, filtersToUse?: typeof filters) => {
    try {
      ...
      // Handle date filters - use data_entry_start_date and data_entry_end_date
      if (activeFilters.reportDays === 'custom' && activeFilters.customDateFrom && activeFilters.customDateTo) {
        queryParams.append('data_entry_start_date', activeFilters.customDateFrom);
        queryParams.append('data_entry_end_date', activeFilters.customDateTo);
        ...
        queryParams.append('data_entry_start_date', fromDate);
        queryParams.append('data_entry_end_date', toDate);

        console.log(`Date range for ${activeFilters.reportDays}: ${fromDate} to ${toDate}`);
      }

      const queryString = queryParams.toString();
      const endpoint = `/cati/qc/data-entry-user-statistics?${queryString}`;
  ...
  const handleDownload = async () => {
    try {
      ...
      if (filters.reportDays === 'custom' && filters.customDateFrom && filters.customDateTo) {
        queryParams.append('data_entry_start_date', filters.customDateFrom);
        queryParams.append('data_entry_end_date', filters.customDateTo);
      ...
      const summaryRows = [
        ...
        '',
        '',
        summary.overall_total_assigned || 0,
        summary.overall_total_complete || 0,
        summary.overall_total_pending || 0
      ]
        ...
        <Container maxWidth="7xl" className="w-full max-w-9xl mx-auto p-6 main-container">
          <div className="flex justify-center items-center h-64">
            <div className="text-center">
              ...
              <Text className="text-gray-600">Loading Data Entry user progress data...</Text>
            </div>
          </div>
        </Container>
      </div>
      ...
      <Card>
        <div className="flex justify-between items-center mb-6">
          <div className="flex items-center">
            ...
            <Heading level={4} className="text-lg font-semibold text-gray-900 dark:text-white">Data Entry User Progress Summary</Heading>
          </div>
          <Button
            variant="primary"
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              ...
              {summary && (
                <tr className="bg-gray-100 font-semibold">
                  ...
                  <td className="px-4 py-4 whitespace-nowrap text-sm font-mono text-gray-700 text-center">Summary</td>
                  <td className="px-4 py-4 whitespace-nowrap text-sm text-gray-700">-</td>
                  <td className="px-4 py-4 whitespace-nowrap text-sm font-mono text-gray-700 text-center">-</td>
                  <td className="px-4 py-4 whitespace-nowrap text-sm font-mono text-gray-700 text-center">{summary.overall_total_assigned.toLocaleString()}</td>
                  <td className="px-4 py-4 whitespace-nowrap text-sm font-mono text-gray-700 text-center">{summary.overall_total_complete.toLocaleString()}</td>
                  <td className="px-4 py-4 whitespace-nowrap text-sm font-mono text-gray-700 text-center">{summary.overall_total_pending.toLocaleString()}</td>
                </tr>
              )}
              {/* end summary section */}
              ...
              {qcUserProgressData.map((user, index) => (
                <tr key={`${user.user_id}-${index}`} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-4 whitespace-nowrap text-sm font-mono text-gray-900 text-center">{user.total_assigned.toLocaleString()}</td>
                  <td className="px-4 py-4 whitespace-nowrap text-sm font-mono text-gray-900 text-center">{user.total_complete.toLocaleString()}</td>
                  <td className="px-4 py-4 whitespace-nowrap text-sm font-mono text-gray-900 text-center">{user.total_pending.toLocaleString()}</td>
                </tr>
              ))}
            </tbody>
          </Table>
