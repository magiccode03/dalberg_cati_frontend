...
export default function NewQCPage() {
  ...
  const fetchInterviews = async (userData?: any, page: number = 1) => {
    try {
      ...
      const apiUrl = `${apiBaseUrl}/api/cati/qc/pending-interviews?qc_teleform_user_id=${currentUserData.teleform_user_id}&page=${page}&limit=${pageSize}`;
      ...
      if (data.success && data.data && Array.isArray(data.data.interviews)) {
        ...
        if (data.data.pagination) {
          ...
          setTotalPages(Math.ceil(pagination.total_count / pageSize));
        ...
        <div className="bg-white">
          <div className="mb-4">
            <p className="text-sm text-gray-600">
              Total <strong>{totalItems.toLocaleString()}</strong> interviews. Teleform User ID: <span className="font-mono font-semibold">{qcTeleformUserId}</span>
            </p>
          </div>
