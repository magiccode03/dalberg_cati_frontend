...
export default function TeleFormV2Page() {
  ...
  const evaluateCondition = (condition: string): boolean => {
    ...
    try {
      ...
      expr = expr.replace(includesPattern, (match, field, value) => {
        ...
        // Handle both single values and arrays for checkbox fields
        if (Array.isArray(fieldValue)) {
          return fieldValue.includes(value).toString();
        } else if (fieldValue !== undefined && fieldValue !== null && fieldValue !== '') {
          // For single value fields (like q10 with maxSelections: 1), check direct equality
  ...
  const handleCheckboxChange = (fieldTag: string, optionValue: string, checked: boolean, field: FormField) => {
    setFormData(prev => {
      ...
      // Convert current value to array format for processing
      ...
      // If maxSelections is 1, return single value instead of array
      const finalValue = maxSelections === 1 ? (newValues.length > 0 ? newValues[0] : '') : newValues;
      
      const newData = { ...prev, [fieldTag]: finalValue };
      
      // Clear validation error for this field when user selects an option
      if (maxSelections === 1 ? finalValue !== '' : newValues.length > 0) {
        setValidationErrors(prev => {
          const newErrors = new Set(prev);
          newErrors.delete(fieldTag);
          return newErrors;
      ...
      if (field.rules?.showFields) {
        Object.entries(field.rules.showFields).forEach(([optTag, fieldsToShow]) => {
          ...
          const isOptionSelected = maxSelections === 1 
    ...
    currentFormConfig.forEach((field) => {
      if (field.required && isFieldVisible(field)) {
        ...
        if (field.type === 'checkbox') {
          // Handle both single values (maxSelections: 1) and arrays (maxSelections > 1)
  ...
  const renderField = (field: FormField, index: number) => {
    ...
    const fieldValue = formData[field.tag] || (field.type === 'checkbox' ? (field.maxSelections === 1 ? '' : []) : '');
    const isEmpty = field.type === 'checkbox' 
      ? (field.maxSelections === 1 
          ? fieldValue === undefined || fieldValue === null || fieldValue === ''
          : !Array.isArray(fieldValue) || fieldValue.length === 0)
