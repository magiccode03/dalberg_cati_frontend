...
const QCUserPage: React.FC = () => {
  ...
  const [searchFilters, setSearchFilters] = useState<SearchFilters>({
    ...
    telecaller: '',
    ac_code: '',
    permission: 'qc', // Default to QC users
  ...
  const [showQCMetrics, setShowQCMetrics] = useState(true); // Show QC metrics by default for QC users
  ...
  const fetchTelecallerOptions = useCallback(async () => {
    ...
    try {
      ...
      // Build params with qc filter
      const params: any = { limit: '1000', qc: '1' };
  ...
  // Optimized fetch QC users with useCallback
  const fetchQCUsers = useCallback(async (page: number = state.currentPage, useDefaultFilter: boolean = false) => {
    ...
    try {
      const params: any = {
        ...
        qc: '1', // Always filter for QC users
      };
      ...
      if (response.ok && result.success) {
        ...
        updateState({ error: result.message || 'Failed to fetch QC users' });
      }
    } catch (err: any) {
      updateState({ error: err.message || 'Error fetching QC users' });
      console.error('Error fetching QC users:', err);
    } finally {
  ...
  useEffect(() => {
    ...
    console.log('Initializing QC user data...');

    const initializeData = async () => {
      // Fetch QC user options and telecalling groups
      await Promise.all([
      ...
      // Then fetch initial QC user data with default filter
      await fetchQCUsers(1, true); // true = use default filter
    };
  ...
  useEffect(() => {
    if (state.currentPage > 1 && !paginationFetched.current) {
      ...
      fetchQCUsers(state.currentPage);
  ...
  const handleSearch = useCallback((e: React.FormEvent) => {
    ...
    // Always show QC metrics for QC users
    setShowQCMetrics(true);

    updateState({ currentPage: 1 });
    fetchQCUsers(1);
  }, [updateState, searchFilters, fetchQCUsers]);
  ...
  const handleClear = useCallback(() => {
    setSearchFilters({
      ...
      telecaller: '',
      ac_code: '',
      permission: 'qc', // Reset to QC users
      telecalling_group_id: '',
    });

    // Keep QC metrics visible
    setShowQCMetrics(true);

    updateState({ currentPage: 1 });
    fetchQCUsers(1);
  }, [updateState, fetchQCUsers]);
  ...
  const handleAddData = useCallback((user: UnifiedUserData) => {
    ...
    updateState({
      selectedTelecaller: { ...selectedTelecaller, type: 'qc_user' },
      isQCModalOpen: true,
    });
  }, [updateState]);
          ...
          <div className="flex flex-col lg:flex-row lg:justify-between lg:items-center gap-4 mb-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:flex lg:space-x-2 gap-2 lg:gap-0">
              ...
              >
                ...
                <span className="hidden sm:inline">Add New QC User</span>
                <span className="sm:hidden">Add New QC User</span>
              </Button>
            </div>
          </div>
          ...
          {state.loading ? (
            ...
            <div className="text-center py-12">
              ...
              <p className="text-gray-500 dark:text-gray-400">
                Try adjusting your search filters or add a new QC user.
              </p>
            </div>
          )}
