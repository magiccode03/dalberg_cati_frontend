...
export default function WizardForm<T extends FieldValues>({
  steps,
  onSubmit,
  onStepChange,
  onComplete,
  ...
  const { handleSubmit, formState: { isValid, isDirty }, watch, trigger, setValue } = form;
      ...
      if (savedData) {
        try {
          ...
          Object.keys(data).forEach(key => {
            setValue(key as keyof T, data[key]);
          });
        } catch (error) {
  ...
  const themeClasses = {
    default: 'bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6',
